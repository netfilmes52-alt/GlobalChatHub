-- ╔══════════════════════════════════════════════════════════════╗
-- ║         GlobalChat Hub v3.1 — Script Único Completo          ║
-- ║  Todos os módulos combinados, sem dependência de GitHub       ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- COMPATIBILIDADE task.* (Delta / mobile)
...
if type(task.spawn)~="function" then task.spawn=(type(spawn)=="function") and spawn or function(f,...) local a={...} local co=coroutine.create(function() f((table.unpack or unpack)(a)) end) coroutine.resume(co) return co end end
if type(task.delay)~="function" then task.delay=(type(delay)=="function") and delay or function(t,f,...) local a={...} task.spawn(function() task.wait(t) f((table.unpack or unpack)(a)) end) end end
...
local function fbDelete(p)
    ...
    httpReq({Url=fbURL(p), Method="PUT", Headers={["Content-Type"]="application/json"}, Body="null"})
end
...
-- checkBan: chamado pela ui.lua após o splash screen
...
-- ALIAS MOBILE (módulos usam G.MOBILE maiúsculo)
GCH.MOBILE = mobile

-- ALIASES mk* — todos os módulos usam G.mkFrame, G.mkLabel etc.
-- Core define como GCH.F, GCH.L, GCH.B — mapeamos aqui:
GCH.mkFrame  = GCH.F
GCH.mkLabel  = GCH.L
GCH.mkButton = GCH.B
GCH.mkCorner = GCH.R
